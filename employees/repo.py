...
async def get_employee_byname(db:AsyncSession,name:str)->Employee:
    stmt = select(Employee).where(Employee.name.ilike(name),Employee.deleted_at.is_(None)).options(
            selectinload(Employee.addresses),
            selectinload(Employee.employee_departments)
            .selectinload(EmployeeDepartment.department),
